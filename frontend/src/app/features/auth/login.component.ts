    ...
    <div class="auth-wrap">
      ...
      <p-card>
        <div class="brand">
          ...
          <div>
            <h2>Field Task Manager</h2>
            <span class="muted">{{ 'auth.login.subtitle' | translate }}</span>
          </div>
        </div>
        ...
        <div class="field">
          ...
          <input pInputText id="email" type="email" [(ngModel)]="email" [placeholder]="'auth.emailPlaceholder' | translate" (keyup.enter)="submit()" />
        </div>
        <div class="field">
        ...
        <p-button [label]="'auth.login.action' | translate" icon="pi pi-sign-in" [loading]="loading" (onClick)="submit()" styleClass="w-full" />

        <p class="muted small">
          {{ 'auth.login.noAccount' | translate }} <a routerLink="/register">{{ 'auth.login.registerLink' | translate }}</a>
        </p>
      </p-card>
    </div>
  `,
...
export class LoginComponent {
  ...
  submit(): void {
    if (!this.email || !this.password) {
      this.error = this.translate.instant('auth.login.validation');
      return;
    }
    this.loading = true;
    this.error = '';
